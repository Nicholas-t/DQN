...
def visualize_cart_distribution(episode):
    df = pd.DataFrame({
        "pole" : [s[0] for s in episode]
    })
    sns.displot(df, x="pole", kind="kde", bw_adjust=.5)
...
def plot_value_function(env, estimator, iterations = 50000, title = "Value function", discount_factor=1.0, alpha=0.01):
    vfn = None
    if "Monte-carlo" in title:
        vfn = estimator(env, iterations=iterations, discount_factor=discount_factor)
    else:
        vfn = estimator(env, iterations=iterations, discount_factor=discount_factor, alpha=alpha)
...
def plot_value_function_mc(ENVIRONMENT = 'CartPole-v1',ITERATIONS = 2000, discount_factor=1.0):
    env = gym.make(ENVIRONMENT)
    _ = plot_value_function(env, MC, iterations=ITERATIONS, title="Monte-carlo ({} iterations) - env:{}".format(ITERATIONS, ENVIRONMENT), discount_factor=discount_factor)

def plot_value_function_td(ENVIRONMENT = 'CartPole-v1',ITERATIONS = 2000, discount_factor=1.0, alpha=0.01):
    env = gym.make(ENVIRONMENT)
    _ = plot_value_function(env, TD, iterations=ITERATIONS, title="Temporal Differences ({} iterations) - env:{}".format(ITERATIONS, ENVIRONMENT), discount_factor=discount_factor, alpha=alpha)